...
public class LightCaster : MonoBehaviour
{
    ...
    [SerializeField] public LayerMask walLayerMask;
    ...
    public bool showRed, showGreen;

    public struct angleVerts
    {
        public Vector3 vert;
        public Vector2 uv;
        public float angle;
  ...
  void Start () {
	...
	void Update()
	{
        ...
        angleVerts[] angleds = new angleVerts[(objverts.Length*2)];
        ...
        Array.Sort(angleds, delegate(angleVerts one, angleVerts two) {
	...
	void GetWalls()
	{
		sceneObjects = Physics.OverlapSphere(transform.position, radius, walLayerMask);
	}
}
